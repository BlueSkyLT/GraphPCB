#!/usr/bin/env python3
"""
Node Classification Script for GraphPCB Dataset

Supports MLP, GCN, GIN, GAT models with command line interface.
Usage:
    python node_classification.py --dataset Graph-F --model GCN
    python node_classification.py --dataset Graph-W --model GAT --epochs 200 --lr 0.001
"""
...
def main():
    parser = argparse.ArgumentParser(description='Node Classification with GraphPCB Dataset')
    ...
    # Model arguments
    parser.add_argument('--model', type=str, required=True, choices=['MLP', 'GCN', 'GIN', 'GAT'],
                       help='Model to use')
    ...
    parser.add_argument('--num-heads', type=int, default=8, help='Number of attention heads (for GAT)')
    ...
    config = {
        "experiment_name": f"{args.model}_{dataset_name}",
    ...
    print(f"Starting training with model {args.model} on dataset {dataset_name}")
